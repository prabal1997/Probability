function [ output ] = rand_viewer( dist_func, inv_dist_func, test_length, unity_interval_count, display_count_x, display_count_y)
%RAND_VIEWER views the DISTRIBUTION function associated with the given data
    ...
    for i = 1:test_length
        %change the 'base' of the random number
        bucket_index = rand_numbers(i) - display_count_x(1);
        
        %finding if the random number even fits into the graph
        if ( bucket_index < 0 || bucket_index > diff(display_count_x) )
            dens_bucket(1) = dens_bucket(1) + (bucket_index < 0);
        else
            %finding the indices of remaining random numbers
            bucket_index = ceil(bucket_index*unity_interval_count)+1;
            dens_bucket(bucket_index) = dens_bucket(bucket_index) + 1;
        end
    end
    dist_bucket = dens_bucket;
    
    %calculating the distribution function
    for i = 1:length(dist_bucket)-1
        dist_bucket(i+1) = dist_bucket(i+1) + dist_bucket(i);
    end
    dist_bucket = dist_bucket/test_length;
