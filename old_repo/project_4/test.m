function [output_vars] = test()
    ...
    for idx = 1:point_count
        ...
        storage_array(idx) = server_simulator(@rt_exp_inv_dist_func, @rt_periodic_inv_distr, 1000);
        storage_array(idx)
        storage_array(idx)/(1000/52)
    end
    
    %count stuff by distributing it among buckets
    bucket_count = 40;
    buckets = zeros(1, bucket_count);
    value_range = 1000*(1/52)*500;
    ...
    %for idx = 2:bucket_count
    %    buckets(idx) = buckets(idx-1) + buckets(idx)
    %end
    %buckets = buckets/point_count;
